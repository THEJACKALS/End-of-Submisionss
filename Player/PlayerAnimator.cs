...
namespace SupanthaPaul
{
	public class PlayerAnimator : MonoBehaviour
	{
		...
		private static readonly int WallLadder = Animator.StringToHash("WallLadder");
		...
		private void Update()
		{
			...
			m_anim.SetBool(WallLadder, m_controller.isClimbing);

			// Attack animation (triggered by left mouse click)
			if (Input.GetMouseButtonDown(0))
			{
				m_anim.SetBool(Melee, true);
			}
			else
			{
				m_anim.SetBool(Melee, false);
			}
		}
	}
}
