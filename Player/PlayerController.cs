...
namespace SupanthaPaul
{
	public class PlayerController : MonoBehaviour
	{
		...
		// Sprint - Tambahan untuk lari dinamis
		[Header("Sprinting")]
		...
		[SerializeField] private int extraJumpCount = 1; // Double jump sudah ada
		...
		// Ladder - Tambahan untuk naik turun tangga
		...
		// Sprint - Variable tambahan untuk Sprint
		[HideInInspector] public bool isSprinting = false;
		// Ladder - Variable tambahan untuk Ladder
		...
		private bool m_hasSprintedInAir = false;
		// Ladder - Variable tambahan untuk Ladder
		...
		void Start()
		{
			...
			// Sprint - Tambahan pool untuk sprint effect
			if (sprintEffect != null)
				PoolManager.instance.CreatePool(sprintEffect, 2);
			...
			// Sprint - Inisialisasi cooldown sprint
			m_sprintCooldown = sprintCooldown;

			m_rb = GetComponent<Rigidbody2D>();
			m_dustParticle = GetComponentInChildren<ParticleSystem>();
			// Ladder - Simpan nilai gravity normal untuk digunakan saat tidak di tangga
		...
		private void FixedUpdate()
		{
			...
			// Ladder - Check if on ladder
			isOnLadder = Physics2D.OverlapCircle(position, ladderCheckRadius, whatIsLadder);
			...
			{
				// Ladder - Handle ladder climbing
				if (isOnLadder && isClimbing)
				{
					// Disable gravity when climbing
					m_rb.gravityScale = 0f;
					// Move up/down based on vertical input
					m_rb.velocity = new Vector2(moveInput * speed * 0.5f, m_verticalInput * climbSpeed);
				}
				else
				{
					...
					else
					{
						...
						{
							// Sprint - Gunakan sprint speed jika sedang sprint
							float currentSpeed = isSprinting ? sprintSpeed : speed;
							m_rb.velocity = new Vector2(moveInput * currentSpeed, m_rb.velocity.y);
							
							// Debug untuk sprint
							if (isSprinting)
							{
								Debug.Log("Sprinting with speed: " + currentSpeed);
							}
						}
						else if(!canMove)
		...
		private void Update()
		{
			...
			// Ladder - Get vertical input for ladder climbing
			m_verticalInput = InputSystem.VerticalRaw();

			if (isGrounded)
			{
				m_extraJumps = extraJumpCount;
				// Sprint - Reset sprint in air flag when grounded
				m_hasSprintedInAir = false;
			}
			...
			if (!isSprinting && !m_hasSprintedInAir && m_sprintCooldown <= 0f)
			{
				// Sprint input
				bool sprintKeyPressed = InputSystem.Sprint();
				if (sprintKeyPressed)
				{
					Debug.Log("Sprint key pressed!");
					isSprinting = true;
					// Effect sprint jika ada
					if (sprintEffect != null)
						PoolManager.instance.ReuseObject(sprintEffect, transform.position, Quaternion.identity);
					
					// Jika player di udara saat sprint
					if (!isGrounded)
					{
						m_hasSprintedInAir = true;
					}
				}
			}
			else if (isSprinting && !InputSystem.Sprint())
			{
				// Berhenti sprint jika tombol sprint dilepas
				isSprinting = false;
				m_sprintCooldown = sprintCooldown;
			...
			// Ladder - Handle climbing input
			if (isOnLadder && Mathf.Abs(m_verticalInput) > 0.1f)
			{
				isClimbing = true;
			}
			else if (!isOnLadder || m_verticalInput == 0)
			{
				isClimbing = false;
			}
			...
			// Ladder - Exit ladder when jumping
			else if (InputSystem.Jump() && isClimbing)
			{
				isClimbing = false;
				m_rb.velocity = new Vector2(m_rb.velocity.x, jumpForce * 0.7f);
			}
		...
		private void OnDrawGizmosSelected()
		{
			...
			// Ladder - Draw gizmos for ladder detection
